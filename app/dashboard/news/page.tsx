...
export default function NewsPage() {
  ...
  return (
    <div className="mx-auto max-w-[1000px]">
      <div className="mb-8">
        <h1 className="text-[26px] font-semibold tracking-tight">Market News</h1>
      ...
      {/* Filters */}
      <div className="mb-8 flex flex-col gap-3.5 rounded-[10px] border border-border bg-card p-4">
        {/* Search */}
        <div className="flex items-center gap-2.5 rounded-[7px] border border-border bg-background px-3.5 py-2.5 transition-colors focus-within:border-primary/40">
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="rounded-[7px] border border-border bg-background px-2.5 py-1.5 text-[13px] outline-none transition-colors focus:border-primary/40"
            >
      ...
      {loadingFirst ? (
        <div className="flex flex-col gap-2.5">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="h-[92px] animate-pulse rounded-[10px] border border-border bg-surface-2/50"
            />
          ))}
        </div>
      ) : isError ? (
          ...
          {items.map((n, i) => (
            ...
            >
              <Link
                ...
                className="group flex flex-wrap items-start gap-3.5 rounded-[10px] border border-border bg-card p-4 transition-all duration-300 hover:-translate-y-1 hover:border-primary/25 hover:shadow-[0_1px_0_0_var(--border),0_16px_36px_-16px_rgba(15,23,42,0.16)]"
              >
                <div className="min-w-[200px] flex-1">
                  ...
                  <div className="mb-1 text-[14px] font-semibold tracking-tight">{n.title}</div>
                  <div className="line-clamp-2 text-[12.5px] leading-relaxed text-text-secondary">
                    {n.summary}
                  </div>
                </div>
                <span className="self-center whitespace-nowrap text-[12.5px] font-medium text-primary opacity-0 transition-opacity duration-200 group-hover:opacity-100">
                  View Analysis
                </span>
              </Link>
            </motion.div>
...
function Empty({ text }: { text: string }) {
  return (
    <div className="grid place-items-center rounded-[10px] border border-dashed border-border bg-surface/50 p-12 text-center">
      <Newspaper className="size-8 text-text-muted" />
      <p className="mt-3 text-[13.5px] text-text-secondary">{text}</p>
    </div>
  );
}
